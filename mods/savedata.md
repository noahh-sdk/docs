...
Pretty much every mod will eventually enter the point where it needs to save some data on the user's machine, such as configuration settings or other user-customizable values. Noahh has built-in support for **two kinds of savedata**: **user-editable settings** and **non-editable saved values**.
...
You can save any type of value as long as it implements `matjson::Serialize` (see the [STL container implementations](https://github.com/noahh-sdk/json/blob/main/include/matjson/stl_serialize.hpp) for an example).

## Setting & getting save data

You can set save data using [setSavedValue](/classes/noahh/Mod#setSavedValue)
...
You can similarly get the current saved value using [getSavedValue](/classes/noahh/Mod#getSavedValue). If the value has not been saved, a default value can be explicitly provided, or otherwise a default value will be created by invoking the save value's default constructor:
...
Mods should save other data (files, backups, etc.) to their specific save directory, which they can access with `Mod::get()->getSaveDir()`. Using Noahh's provided directories ensures that when the user uninstalls the mod, Noahh can properly uninstall all its data. Noahh's provided directories are also guaranteed to be readable and writable on all platforms.
