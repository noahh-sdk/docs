...
> :warning: **Settings are being fully reworked**. See [the docs page for the new system](/mods/settings-v3) for more information. The URL for this page will be changed to `/mods/settings-old` in the future, once the new system has been released.

Noahh mods can have settings, specified in `mod.json`. These settings are automatically saved and loaded, and an UI is produced for the user to edit them in-game through the mod's page.
...
You can get the current value of a setting using the [getSettingValue](/classes/noahh/Mod#getSettingValue) method. Note that the type you get the value as must match the value type of the setting type - see [Supported setting types](#supported-setting-types) for more info.
...
If the existing options aren't enough, mods may also provide custom settings. In this case, the mod needs to define two classes for Noahh: a class that holds the setting's value, and a node class that represents it in the UI. As Noahh does not know the UI or values for custom settings unless the mod is loaded, these settings can not be edited if the mod is disabled.
...
The first class that the mod needs to provide for the setting is one that inherits [SettingValue](/classes/noahh/SettingValue). This class manages the current value of the setting as well as saving and loading that value.
...
The second class the mod needs to provide is one that inherits [SettingNode](/classes/noahh/SettingNode). This should be returned by the call to `MySettingValue::createNode`. It is used by Noahh to display the setting's UI. Note that the node should only set the value of the setting when `commit` is called; this allows users to undo accidental changes.

The setting node is provided the current width of the setting layer available in its `init` function; the height of the setting may be as large or as small as it needs. **You must set the content size of the setting node** in order to let Noahh know the height of your setting.

Whenever the value of the setting is changed using the node's controls, the node should call the `dispatchChanged` method to let Noahh's UI know.
...
protected:
    bool init(MySettingValue* value, float width) {
        ...
        // Set up the UI. Note that Noahh provides a background for the
...
public:
    ...
    // Noahh calls this to query if the setting value has been changed,
    // and those changes haven't been committed
    bool hasUncommittedChanges() override {
        // todo
    }

    // Noahh calls this to query if the setting has a value that is
    // different from its default value
    bool hasNonDefaultValue() override {
        // todo
    }

    // Noahh calls this to reset the setting's value back to default
...
The last this the mod needs to do is register their setting value class to Noahh. This should be done immediately when the mod is loaded, to ensure that the value of the setting is loaded from disk on time. Registering can either be done using the raw [registerCustomSetting](/classes/noahh/Mod#registerCustomSetting) method, or the [addCustomSetting](/classes/noahh/Mod#addCustomSetting) convenience function.
