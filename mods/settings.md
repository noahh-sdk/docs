...
> :warning: **Settings are being reworked. This docs page now details how to use the new (CURRENTLY UNRELEASED) system.** The docs page for the old system is available at [/mods/settings-old.md].

Mods will often want to have properties that the user can configure, such as how often a backup should be made or if a menu redesign should be enabled. For this, Noahh provides the **Settings** system for a standardized and simple way to create user-editable properties that are preserved between bootups.
...
Once you have defined your settings, you can use them in your mod via the [`Mod::getSettingValue`](/classes/noahh/Mod#getSettingValue) function:
...
| `enable-if-description` | String | Human-readable description for what the `enable-if` scheme does. If not provided, Noahh will try to synthesize a legible one from the `enable-if` scheme itself. However, specifying a custom description is heavily recommended if you use saved values, or have more complex schemes |
...
![An image showcasing the custom enum setting in action in Noahh](/assets/settings/custom-enum.png)
...
struct matjson::Serialize<MyCustomEnum> {
    // See https://github.com/noahh-sdk/test-mod/blob/main/src/MyCustomSettingV3.cpp#L12-L30 
    // for the definition
};
...
public:
    ...
    static Result<std::shared_ptr<MyCustomSettingV3>> parse(
        ...
        // It is recommended to use the JSON checking utilities in Noahh for 
        ...
        // want to use Noahh's JSON checking utilities, as well as a fourth bool 
...
template <>
struct noahh::SettingTypeForValueType<MyCustomEnum> {
...
![An image showcasing the custom button setting in action in Noahh](/assets/settings/custom-button.png)
