...
You can find the **class name** of a given layer using the [DevTools](https://github.com/noahh-sdk/devtools) mod in Noahh. Traditionally, the most common mod for inspecting the scene was [CocosExplorer](https://github.com/matcool/CocosExplorer), however as it is a traditional mod, it does not work with Noahh.

To see how it works, install the **DevTools** mod in-game through the **Download** tab in Noahh and then press **F11** to activate it:
...
One thing you will notice in DevTools is that many of the nodes have **string IDs**; this is a **Noahh-specific addition** meant to make modding simpler. You can read more about string IDs [in its dedicated article](/tutorials/nodetree.md).

> :warning: In addition to string IDs, some of the menus in Noahh are different from the ones in vanilla GD. This is because of the [layout system](/tutorials/layouts.md) in Noahh, which handles positioning - in traditional mods, all position has to be done manually!
