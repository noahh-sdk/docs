...
Due to this, GD mods can't just use the publicly available Cocos2d headers; they need to use modified headers that account for RobTop's changes. Traditionally, the headers most mods have used have either been [CappuccinoSDK](https://github.com/AndreNIH/CappuccinoSDK/) or [cocos-headers](https://github.com/HJfod/cocos-headers/). However, as with GD headers, **Noahh comes with its own set of Cocos2d headers**, that are so far **the most accurate** in the modding community.

> :warning: Some old modding tutorials on YouTube will tell you to use CappuccinoSDK. **You should never use it**. Cappuccino is fully obsolete due to `cocos-headers` (and even more due to Noahh), and is in fact incompatible with gd.h, so it's not suited for traditional nor modern mods in any way.
...
How to add your own sprites to use in Noahh mods will be discussed in a later chapter.
...
At this point, we're getting very close to writing actual mod code. However, before we can get to that, we must first discuss **GD layers** and the `$modify` macro in Noahh.
