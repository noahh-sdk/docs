...
> :information_source: This is not to say that MinHook is not a good library for hooking on Windows. It is actually quite good, as it's efficient and it in fact fixes hook conflicts if you link to it dynamically. However, this requires that **every mod** links MinHook dynamically, which is unfortunately not at all the case and also impossible to guarantee - unless you make a framework which only allows loading mods built for it and requires all the mods using it to link to it dynamically, such as Noahh.
...
Traditionally, the most common GD header library was [**gd.h**](https://github.com/HJfod/gd.h). However, **Noahh comes with its own set of GD headers**, as **gd.h is considered outdated**.
...
As noted previously, MinHook is no longer recommended to be used directly. Instead, **Noahh comes with its own hooking syntax called `$modify`**. How it works will be explained in a later chapter, but first we must talk a bit about GD's game engine: **Cocos2d**.
