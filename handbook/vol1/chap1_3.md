...
The main way to create hooks in Noahh is using an abstraction called `$modify` - however, it is a very powerful tool and hard to explain without some preface. It is also just an abstraction; you can create hooks manually in Noahh as well through the [`Mod::addHook`](/classes/noahh/Mod#addHook) interface - although in practice **you should never be creating manual hooks unless necessary**.
...
Let's see a [real-world example](/tutorials/manualhooks) of creating manual hooks in Noahh:
```cpp
auto wrapFunction(uintptr_t address, tulip::hook::WrapperMetadata const& metadata) {
	auto wrapped = noahh::hook::createWrapper(reinterpret_cast<void*>(address), metadata);
	if (wrapped.isErr()) {{
...
void MenuLayer_onNewgrounds(MenuLayer* self, CCObject* sender) {
	...
	static auto original = wrapFunction(
        noahh::base::get() + 0x27b480,
        tulip::hook::WrapperMetadata{
            .m_convention = noahh::hook::createConvention(tulip::hook::TulipConvention::Thiscall),
...
$execute {
    Mod::get()->addHook(
        reinterpret_cast<void*>(noahh::base::get() + 0x27b480),
...
The most important part of this code is `noahh::base::get() + 0x27b480`. This is the **address** of the function. When C++ is compiled down to machine code, **all variable and function names are erased** and functions are instead given **memory addresses**. A memory address is just **the location in a binary that the function resides in**. For example, `noahh::base::get() + 0x27b480` means that the function is located at offset `0x27b480` (or 2602112 in decimal) bytes from the **base address** - GD's base address, given by the function `noahh::base::get()`.
...
> :warning: The above code snippet also references **calling conventions** - if you use `$modify`, Noahh handles them for you, however if you're doing manual hooks or reverse engineering, these are very important to get right.

## Addresses

So how do we find out these things? Usually, this is done through **reverse engineering**; however, RE is quite a complex skill, and would take far too much time to explain here, so it has its [own dedicated volume instead](https://docs.noahh-sdk.org/handbook/vol2/chap2_1). And on top of that, **most common functions have already been found**. This means that instead of REing the function yourself, you can use the GD bindings that come packaged with Noahh.
...
As noted previously, you shouldn't usually be creating hooks manually. Instead, **Noahh comes with a special hooking syntax called `$modify`**. How it works will be explained in a later chapter, but first we must talk a bit about GD's game engine: **Cocos2d**.
