...
If you're using some IDE you may want to do a few things for your Noahh projects.
...
- `C/C++`
- `CMake Tools`
- `Noahh` - Not required, but will provide some nice features.
...
You **must** build your mod first so that errors such as `#include <Noahh/modify/MenuLayer.hpp> not found` go away.
...
If the toolchain and/or splat were installed **not** via `noahh sdk install-linux`, modify the paths accordingly.
...
If there are errors similar to VSCode (such as `#include <Noahh/modify/MenuLayer.hpp> not found`) after you've built, restarting Visual Studio should make them go away.

If you get an error about Noahh needing to be compiled for 32-bit, that means you didn't change your toolset to x86 above.
...
The build should end with a message saying `Build finished` (assuming you ran `noahh config setup` before) and the empty mod \
