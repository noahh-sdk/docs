...
You can use one of the following IDEs for developing Noahh mods. Using an IDE is highly recommended, as they will give you code completion, intellisense, and simplify developing mods. Our recommended IDE is VS Code, which we also have a [dedicated extension](https://marketplace.visualstudio.com/items?itemName=NoahhSDK.noahh) for.
 ...
 * [`Noahh`](https://marketplace.visualstudio.com/items?itemName=NoahhSDK.noahh) - Not required, but will provide some nice features.
...
Additionally, we recommend disabling clangd auto header insertion, as it doesn't work well with Noahh and will spam useless headers in your project. Press F1 and run `Preferences: Open User Settings (JSON)` and add the following lines:
```json
"clangd.arguments": [
...
```json
{
    "name": "Noahh Windows 64-bit",
    "compilers": {
        "C": "/usr/bin/clang",
        "CXX": "/usr/bin/clang++"
    },
    "isTrusted": true,
    "toolchainFile": "${userHome}/.local/share/Noahh/cross-tools/clang-msvc-sdk/clang-msvc.cmake",
    "cmakeSettings": {
        "HOST_ARCH": "x86_64",
        "SPLAT_DIR": "${userHome}/.local/share/Noahh/cross-tools/splat",
        "CMAKE_EXPORT_COMPILE_COMMANDS": 1
    }
}
```

If the toolchain and/or splat weren't installed via `noahh sdk install-linux`, modify the paths accordingly.
...
* `"toolchainFile"` to `${userHome}/.local/share/Noahh/cross-tools/clang-msvc-sdk/clang-cl-msvc.cmake`
...
If there are errors similar to VS Code (such as `#include <Noahh/modify/MenuLayer.hpp> not found`) after you've built, restarting Visual Studio should make them go away.
...
![Image showing the CLion Open Project Wizard set up for Noahh](/assets/clion_openprojectwizardsetup.png)
...
The build should end with a message saying `Build finished` and (assuming you ran `noahh config setup` before) the mod should now be installed to Geometry Dash.
...
```bash
export NOAHH_SDK=$HOME/Documents/Noahh
export SPLAT_DIR=$HOME/.local/share/Noahh/cross-tools/splat
export CMAKE_TOOLCHAIN_FILE=$HOME/.local/share/Noahh/cross-tools/clang-msvc-sdk/clang-msvc.cmake
export HOST_ARCH=x86_64
```

If the toolchain and/or splat weren't installed via `noahh sdk install-linux`, modify the paths accordingly.

Save it as `noahh-env.sh` in your home directory (or wherever you want, just remember the path).
