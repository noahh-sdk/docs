...
Noahh will also look for these special files within your mod folder:
...
> Check out `noahh build --help` for other options!

If you have an issue running that command for whatever reason ([do let us know!](https://github.com/noahh-sdk/cli/issues)), you can build your mod the same way using these commands:
```bash
# Configure CMake
cmake -B build
...
If you [created a profile in CLI](/getting-started/noahh-cli), then the mod should be automatically installed to GD. If not, then the built `your.mod.noahh` package should be in your build folder, from where you can manually install it in-game.
...
You must also get the Noahh binaries for Android, you can do this using the CLI:
```bash
noahh sdk install-binaries -p android64
# Or if you're using a 32 bit phone
noahh sdk install-binaries -p android32
```

Now you can build your mod for Android via:
```bash
noahh build -p android64
# Or if you're using a 32 bit phone
noahh build -p android32
```

You can then copy the built .noahh file from the `build-android64` folder to your phone at this location:
```
/storage/emulated/0/Android/media/com.noahh.launcher/game/noahh/mods/
```

## Building Windows mods on Linux

If you have followed the steps earlier and installed all the required tools with `noahh sdk install-linux`, building should be as simple as on Windows:
...
```bash
noahh build -- -DCMAKE_TOOLCHAIN_FILE=/path/to/clang-msvc-sdk/clang-msvc.cmake -DSPLAT_DIR=/path/to/splat
```