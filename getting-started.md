...
icon: play
description: Instructions for creating mods with the Noahh SDK & other developer tools
order: 1
---

# Getting started

Make sure to [install Noahh](/installation.md) first.

## Creating a new mod

Navigate to some projects folder on your computer and run `noahh new` from the terminal. Follow the prompts given to initialize a new directory with template mod code, and then open up the created directory in your favorite code editor.

The structure of a Noahh mod is the following:
 ...
 * `CMakeLists.txt` is for configuring CMake. The default configuration provided by `noahh new` should cover most smaller mods
...
See also [our tutorial](/tutorials/migrating.md) for porting traditional mods to Noahh.

The [Tutorials](/tutorials) category in general is a great source for information about working with Noahh mods.
