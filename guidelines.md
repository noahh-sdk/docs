...
Any mod found to be harmful will be publicly reported as such and will be warned against. Its developer may also receive a temporary or permanent ban from the `mods` repository, or in severe cases off all Noahh-related platforms.
...
## 2. Use Noahh appropriately

Write reasonable code, basically.

### 2.1. Use Noahh's solutions (if they exist)

If Noahh has a solution to a problem, use that and do **not** create your own solution to it. If you want to add custom keybinds to your mod, use the Noahh API. If you feel the solution Noahh provides is insufficient for your needs, please [let us know](/docs/contributing) and we'll look into expanding it.

If Noahh comes with a solution for adding nodes to a certain layer, use that instead of hooking the layer yourself.

Do note that "Noahh" here includes the loader and the official API mod.

### 2.2. Be careful with open access

Noahh is built as an open framework where any mod can access basically any data it needs, but this doesn't mean that they *should* access it. For example, modifying another mod's `Mod` class should only be done in situations where they have intended for that to be done.

### 2.3. Follow Noahh's conventions when using it
...
These are guidelines that could apply to non-Noahh mods aswell.
