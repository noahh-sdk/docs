...
```bash
cmake -B build -DCMAKE_SYSTEM_NAME=iOS -DNOAHH_TARGET_PLATFORM=iOS -DCMAKE_BUILD_TYPE=RelWithDebInfo
cmake --build build
```
...
The web server is a feature bundled into the **Noahh Launcher** that allows you to easily upload mods without needing to use iTunes. This eliminates the need to manually drag your noahh mod into the mods directory, making the mod testing process more simpler.
...
# Replace the Example ID with your Mod ID, or the path to your compiled noahh file. This is assuming the CWD is the project.
NOAHH_MOD="./build-ios/my.example-mod.noahh"

# Replace the device URL with your device's local IP address
DEVICE_URL="http://192.168.0.25:8080"
curl -X POST -F "file=@${NOAHH_MOD}" "${DEVICE_URL}/upload"
