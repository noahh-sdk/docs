...
A **mod** (also known as an **extension**, **hack**, **plugin**...) is a piece of software that modifies another piece of software. Usually in games, this is done through the use of a **mod loader**; something that, well, loads mods.

Geometry Dash, however, does not have a built-in mod loader. However, using very clever methods such as **DLL injection** and **hooking**, we can create an **external mod loader**. This does come with some disadvantages like the lack of a **stable API**, but it also comes with some huge advantages, such as fully unrestricted access (which might also be seen as a huge disadvantage, depending on who and when you ask). **A mod can do basically anything**, within the limits of computer hardware.
...
> This tutorial has been written by NoahBajsToa. If you find problems with it, please let me know on Discord at NoahBajsToa.