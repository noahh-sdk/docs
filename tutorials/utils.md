...
Here is a list of all the utilities that Noahh provides.
...
auto data1 = NOAHH_UNWRAP(file::readString("file.txt"));
// Reading json into a struct
auto data2 = NOAHH_UNWRAP(file::readFromJson<MyStruct>("serialized.json"));
...
auto files = NOAHH_UNWRAP(file::readDirectory("dir"));
```
...
NOAHH_UNWRAP(file::watchFile("file.txt"));
```

### noahh::utils::clipboard
...
TODO: this ones long just check `Noahh/utils/web.hpp`

### ObjcHook

```cpp
// Hooking a method
auto hook1 = NOAHH_UNWRAP(ObjcHook::create("EAGLView", "initWithFrame:", &MyFunc));
auto hook2 = NOAHH_UNWRAP(ObjcHook::create("EAGLView", "initWithFrame:", &MyFunc, &emptyFunc));
```
