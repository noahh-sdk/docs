# Migrating from Noahh v4.x to v5.0

## Changes to `Popup`

`noahh::Popup` is no longer templated, and instead accepts its own arguments in `Popup::init` (which also replaces `initAnchored`). It now uses the pattern similar to any other node. For example the following code:
