...
Since Noahh v3.4.0, mods are able to also modify the Noahh UI itself in limited ways. This is so mods can do things like add extra buttons to their own mod's popup, make their mod logo have particles, etc. This tutorial shows how to use these APIs to modify Noahh's own UIs.

## Listening for UI events

Noahh currently exposes three UI events through the `<Noahh/ui/NoahhUI.hpp>` header: [`ModPopupUIEvent`](/classes/noahh/ModPopupUIEvent), [`ModItemUIEvent`](/classes/noahh/ModItemUIEvent), and [`ModLogoUIEvent`](/classes/noahh/ModLogoUIEvent). These are for being notified of whenever a mod popup is created, a mod list mod item is created, and a mod logo is created respectively.

Note that **all Noahh UI events may and will be posted _multiple times_!** This is because Noahh UIs often have an initial created "loading" state, and then fetch data from the Noahh servers that is updated onto the created node asynchronously. To allow mods to be notified of when this data is loaded, **the UI events are reposted whenever the state of the UI node changes**. For this reason, **mods can never add nodes without checking if they already exist first**.

You can listen to these events by using Noahh's events system as usual, using the [`EventFilter<T>`](/classes/noahh/EventFilter) helper class. Note that you should always return `ListenerResult::Propagate` to allow other mods to modify the layer as well, like calling the original in a `$modify` hook!

## Guidelines

There are some guidelines on what you are and are not allowed to do when modifying the Noahh UI. These are:
...
$execute {
    new EventListener<EventFilter<ModLogoUIEvent>>(+[](ModLogoUIEvent* event) {
        ...
        // You should always propagate Noahh UI events
        return ListenerResult::Propagate;
    });
    new EventListener<EventFilter<ModItemUIEvent>>(+[](ModItemUIEvent* event) {
        if (event->getModID() == "noahh.loader") {
            // Find the Noahh nodes you want to access first
