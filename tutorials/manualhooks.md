...
$execute {
    Mod::get()->hook(
        reinterpret_cast<void*>(noahh::base::get() + 0x191E90), // address
...
This example shows how to hook a function that is linked to, for example through DLL imports. noahh comes with the `addresser` namespace for utilities related to figuring out the addresses of linked functions, including class methods and virtual functions.
...
$execute {
    Mod::get()->hook(
        reinterpret_cast<void*>(
            ...
            noahh::addresser::getNonVirtual(
                // This is used because this function is overloaded,
                // otherwise just a regular function pointer would suffice (&foobar)
                noahh::modifier::Resolve<const cocos2d::CCPoint&, float, float, unsigned int, bool>::func(&cocos2d::ccDrawCircle)
            )
        ),
