...
[Coroutines](https://en.cppreference.com/w/cpp/language/coroutines) are an underutilized feature of C++20 that not many understand. Under the hood, they are very complex, but luckily Noahh makes it quite simple for you. Noahh lets you leverage the power of coroutines to write clean asynchronous code, tackle Result propagation, and build Python-style generators with ease.

## Task and CoTask

For most asynchronous tasks, Noahh provides the Task class. See [`Tasks`](/tutorials/tasks) for more information. Any function that returns a Task can be converted into a coroutine by simply using `co_await` on a different task.

Here's an example of a simple coroutine:

```cpp
#include <Noahh/utils/web.hpp>
#include <Noahh/utils/coro.hpp>
...
The most annoying part of working with Results is propagation. In Rust, it's as easy as using the `?` operator, but we don't have such nice things in C++. Lucky for us, Noahh implements coroutines for Result that allow for easy propagation:
