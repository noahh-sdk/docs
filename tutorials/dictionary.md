...
An old modding framework that merged with [Lilac](#lilac) and formed [Noahh](#noahh).
...
> Related words: [Noahh](#noahh), [Traditional modding](#traditional-modding)
...
The game engine used by GD. The version used by GD is Cocos2d-x v2.2.3, however with many propietary modifications by RobTop, meaning that you need to use a specially prepared version that accounts for these changes such as [Noahh](#noahh) or [cocos-headers](#cocos-headers) in order to make mods.
...
> [Homepage](https://github.com/noahh-sdk/noahh)
...
> Related words: [Noahh](#noahh), [MinHook](#minhook), [Race Condition](#race-condition)
...
An old modding framework that merged with [Cacao](#cacaosdk) and formed [Noahh](#noahh).
...
A program that modifies GD by adding, changing, and removing content using various techniques such as [hooking](#hooking) and [patching](#patching). [Traditionally made](#traditional-modding) using libraries such as [gd.h](#gdh) and [cocos-headers](#cocos-headers), but may also be made using a framework such as [Noahh](#noahh).
...
Mods made and modding done without a dedicated framework like [Noahh](#noahh), usually using a combination of [gd.h](#gdh), [cocos-headers](#cocos-headers), and [MinHook](#minhook).
