...
class $modify(cocos2d::CCLabelBMFont) {
    static void onModify(auto& self) {
        if (!self.setHookPriorityPost("cocos2d::CCLabelBMFont::init", Priority::First)) {
            noahh::log::warn("Failed to set hook priority.");
        }
        if (!self.setHookPriorityPre("cocos2d::CCLabelBMFont::setString", Priority::Late)) {
            noahh::log::warn("Failed to set hook priority.");
        }
        if (!self.setHookPriorityAfterPost("cocos2d::CCLabelBMFont::limitLabelWidth", "noahh.node-ids")) {
            noahh::log::warn("Failed to set hook priority.");
        }
    }
};
...
- While the current priority is preserved across function calls, it is not preserved in other situations. For example:
  ...
  Instead of calling the original, this will call the hook again. This also applies for calling the original through a Task (see [noahh#994](https://github.com/noahh-sdk/noahh/issues/994)).
...
- Manual hooks can have their priorities set through the [Hook::setPriority](/classes/noahh/Hook/#setPriority) method.
